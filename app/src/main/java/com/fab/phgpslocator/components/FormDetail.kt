...
fun FormDetail(
    ...
    //val listPhoto by databaseViewModel.photoDetail.collectAsState()
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.SpaceEvenly) {
            Button(
                onClick = {
                    val t = mainViewModel.uiState.value.title
                    val d = mainViewModel.uiState.value.description
                    val p = PhotoDetail(id = 2, title = t, description = d)
                    databaseViewModel.insertPhotoDetailFromDB(photo = p)
                },
                modifier = Modifier
                    .padding(16.dp),
